...
import { closedShiftKeys, nextOpenSlot, checkoutBlockMessage } from "./schedule-exceptions.js";

const DAY_MINUTES = 1440;
const SLOT_STEP_MINUTES = 15;
const PREP_MINUTES_GREEN = 60;
const PREP_MINUTES_YELLOW_RED = 30;
const PREP_MINUTES_PICKUP = 15;

// §12/§12b: ciò che differisce tra Delivery e Ritiro nel calcolo slot. Delivery
// è il default storico: chiamare le funzioni senza config (o con DELIVERY_CONFIG)
// riproduce §12 bit-per-bit. Ritiro (§12b): preparazione 15 min in ogni fase
// (nessun vincolo rider) e ultimo slot = orario di chiusura incluso.
const DELIVERY_CONFIG = {
  prepGreen: PREP_MINUTES_GREEN,
  prepYellowRed: PREP_MINUTES_YELLOW_RED,
  closingInclusive: false,
};
const PICKUP_CONFIG = {
  prepGreen: PREP_MINUTES_PICKUP,
  prepYellowRed: PREP_MINUTES_PICKUP,
  closingInclusive: true,
};
...
function computeFirstSlotMinute(linearWindows, nowMinuteOfDay, phase, config = DELIVERY_CONFIG) {
  ...
  if (phase === "green" && current) {
    const candidate = ceilToQuarterHour(nowMinuteOfDay + config.prepGreen);
    // Ritiro (chiusura inclusa): candidate == chiusura resta nella finestra
    // corrente; Delivery (chiusura esclusa): passa alla finestra successiva.
    const withinCurrent = config.closingInclusive
      ? candidate <= current.closesMinute
      : candidate < current.closesMinute;
    if (withinCurrent) return candidate;

    const next = linearWindows.find((w) => w.opensMinute >= current.closesMinute);
    return next ? next.opensMinute + config.prepYellowRed : null;
  }
...
function slotsForOffset(linearWindows, offset, firstSlotMinute, config = DELIVERY_CONFIG) {
  const result = [];
  for (const w of linearWindows) {
    if (w.offset !== offset) continue;
    // Delivery: `m < closesMinute` (chiusura esclusa, §12) — identico
    // all'originale su minuti interi. Ritiro: `m <= closesMinute` (chiusura
    // inclusa, §12b).
    for (
      let m = w.opensMinute;
      config.closingInclusive ? m <= w.closesMinute : m < w.closesMinute;
      m += SLOT_STEP_MINUTES
    ) {
...
function getScheduledSlots(windowRows, referenceDate = new Date(), exceptions = [], config = DELIVERY_CONFIG) {
  ...
  const firstSlotMinute = computeFirstSlotMinute(linearWindows, minuteOfDay, phase, config);
  if (firstSlotMinute === null) {
    return { ...status, ...empty };
  }

  const slots = {
    today: slotsForOffset(linearWindows, 0, firstSlotMinute, config),
    tomorrow: slotsForOffset(linearWindows, 1, firstSlotMinute, config),
  };
...
// §12b: slot per il Ritiro. Riusa interamente getScheduledSlots con
// PICKUP_CONFIG (prep 15 min, chiusura inclusa): nessuna riscrittura della
// logica di finestre/eccezioni (§46b). Il Ritiro non ha ASAP, quindi il
// checkout è bloccato quando non c'è alcuno slot oggi/domani; il messaggio di
// blocco riusa la "prossima apertura utile" di nextOpenSlot (stessa fonte del
// blocco Delivery §68.4). firstSlotDay/firstSlotLabel arrivano già coerenti da
// getScheduledSlots (fix Task C): non vanno ricalcolati qui.
function getPickupSlots(windowRows, referenceDate = new Date(), exceptions = []) {
  const result = getScheduledSlots(windowRows, referenceDate, exceptions, PICKUP_CONFIG);
  const slots = result.slots ?? { today: [], tomorrow: [] };
  const hasSlots = (slots.today?.length ?? 0) > 0 || (slots.tomorrow?.length ?? 0) > 0;
  const checkoutBlocked = !hasSlots;
  const blockMessage = checkoutBlocked
    ? checkoutBlockMessage(nextOpenSlot(referenceDate, exceptions, windowRows))
    : null;

  return {
    slots,
    firstSlotDay: result.firstSlotDay,
    firstSlotLabel: result.firstSlotLabel,
    checkoutBlocked,
    blockMessage,
  };
}
...
export { getScheduledSlots, getPickupSlots, computeScheduledDeliveryAt, ceilToQuarterHour };
