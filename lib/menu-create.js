...
// §63-64 (Fase 4) / §19 / §31 — IL TITOLO DEL GRUPPO DI SCELTA
// (`product_choice_options.choice_label`), decisione di Andrea del 12/08/2026
// ("YY"): si può SCRIVERE, e arriva precompilato con questa frase.
//
// ⚠️ **Il predefinito del DATABASE è `'Proteina'`**, e non è quello che i
// clienti leggono oggi: i Roll esistenti portano *"Come preferisci il tuo
// kebab?"* (§19 v20). Senza questa costante un Roll creato dal pannello
// mostrerebbe un titolo **diverso da tutti gli altri prodotti del menu**, e la
// differenza si vedrebbe solo aprendo quella card.
//
// ⚠️ La frase è **la stessa che il sito già usa come ripiego** (`app/page.js`,
// `{config.choiceLabel ?? "Come preferisci il tuo kebab?"}`): sono due copie
// della stessa stringa e vanno tenute insieme. *Non è stata importata da lì
// perché quel file è un componente client e importarlo qui trascinerebbe React
// dentro una lib; la copia è dichiarata, non nascosta.*
// ⚠️ ESPORTATA il 13/08/2026: la MODIFICA delle opzioni
// (`lib/menu-options-editor.js`) ripiega sulla stessa frase quando si danno
// proteine a un articolo che non ne aveva (decisione "B" di Andrea del 13/08).
// *Esportarla invece di ricopiarla è il punto: due copie della stessa frase
// divergono, ed è già costato tre volte in questo progetto. Le copie dichiarate
// restano due — questa e il ripiego di `app/page.js` — e non tre.*
export const CHOICE_LABEL_DEFAULT = "Come preferisci il tuo kebab?";
// Stesso limite del nome dell'articolo: è un titolo che va a schermo sopra le
// scelte, e più lungo di così scomporrebbe la card. ⚠️ Scelta di chi ha scritto
// questo codice, non una decisione presa a voce: la colonna è `text` e non ha
// limiti suoi.
...
const COLONNE = [
  ...
  "spice_label",
  "is_available",
  // ⚠️ Aggiunta il 13/08/2026 con lo scudo: è la colonna che l'articolo con
  // opzioni abbassa alla nascita, quindi dev'essere leggibile nella risposta.
  "is_in_menu",
  "is_vegan",
  "is_vegetarian",
...
// §63-64 (Fase 4, 12/08/2026, "YY") — IL TITOLO DEL GRUPPO DI SCELTA, uno per
// tutte le proteine dell'articolo.
//
// ⚠️⚠️ **È UN TITOLO PER GRUPPO, NON PER SINGOLA PROTEINA.** Le righe di
// `product_choice_options` hanno una colonna `choice_label` ciascuna, ma
// descrivono **una domanda sola** — *"Come preferisci il tuo kebab?"* — e il
// sito legge quella della **prima riga** (`app/page.js`:
// `choiceLabel: choices[0]?.choice_label`). Righe con titoli diversi non
// darebbero due domande: darebbero **quella che capita per prima**, e quale
// sia dipende dall'ordine. Perciò si rifiuta.
//
// ⚠️⚠️ **QUESTA ETICHETTA NON È COME LE RIMOZIONI, E LA DIFFERENZA VA SAPUTA.**
// La regola DD ("si aggiunge e si toglie, non si rinomina") vale per le
// etichette che il **checkout cerca per nome** — proteina, contorno,
// accompagnamento, rimozioni — dove un rinomino fa rifiutare al pagamento i
// carrelli già composti. `choice_label` no: `lib/checkout-resolve.js` la
// **COPIA** dentro `order_items.configuration` (`configuration.choiceLabel =
// choice.choice_label`) e non la cerca mai — verificato il 12/08, non esiste un
// solo `.eq("choice_label", …)` in tutto `app/` e `lib/`. **Rinominarla non
// rompe nessun carrello**: cambia solo la domanda che i clienti nuovi leggono.
// *Scritto qui perché a qualcuno verrà naturale applicarle la regola DD credendo
// di fare bene, e vietare un rinomino innocuo costa la stessa fatica di
// permetterne uno pericoloso.*
//
// Titolo assente su tutte le righe: vale il predefinito, mai quello del
// database. Presente su alcune e assente su altre: non è un conflitto — chi
// omette non sta proponendo un titolo diverso.
//
// Ritorna { ok: true, valore } oppure { ok: false, error }.
function titoloDelGruppo(proteineGrezze) {
  const righe = Array.isArray(proteineGrezze) ? proteineGrezze : [];
  const titoli = [];

  for (const riga of righe) {
    const grezzo = riga?.choice_label;
    if (grezzo === undefined || grezzo === null || grezzo === "") continue;
    if (typeof grezzo !== "string" || grezzo.trim() === "") {
      return fail("Il titolo della scelta non è valido: scrivi una domanda, oppure lascialo come proposto.");
    }
    const pulito = grezzo.trim();
    if (pulito.length > CHOICE_LABEL_MAX) {
      return fail(`Il titolo della scelta non può superare i ${CHOICE_LABEL_MAX} caratteri.`);
    }
    if (!titoli.includes(pulito)) titoli.push(pulito);
  }

  if (titoli.length === 0) return { ok: true, valore: CHOICE_LABEL_DEFAULT };
  if (titoli.length > 1) {
    return fail(
      `Il titolo della scelta è uno solo per tutto il gruppo, e qui ne sono arrivati ${titoli.length} ("${titoli.join(
        '", "'
      )}"): il cliente legge una domanda sola.`
    );
  }
  return { ok: true, valore: titoli[0] };
}
...
export async function createProductCore({
  ...
  // §63-64 (Fase 4, "YY") — il titolo del gruppo. ⚠️ Si legge dal payload
  // GREZZO e non da `opt`, perché `lib/menu-options.js` non lo conosce: quel
  // modulo è committato e in questo giro non si tocca, quindi la validazione del
  // titolo vive qui. *È un'asimmetria dichiarata, non una svista: se un domani
  // il titolo diventasse un dato per riga, il posto giusto sarebbe là dentro,
  // insieme alle altre regole delle opzioni.*
  const titolo = titoloDelGruppo(payload?.options?.proteins);
  if (!titolo.ok) {
  ...
  //      — e, se l'articolo ha opzioni, **FUORI DAL MENU** (`is_in_menu: false`)
  ...
  //   5. il RIENTRO IN MENU: `is_in_menu: true`, e solo se i passi 1-4 sono andati
  ...
  // l'articolo resta FUORI DAL MENU.**
  //
  ...
  // scelta della carne che il cliente può comprare. **Fuori dal menu**, invece,
  // lo si vede nel pannello e nessun cliente lo incontra.
  //
  // ⚠️ **Per questo l'articolo con opzioni NASCE fuori dal menu e ci rientra
  // alla fine**, invece di nascere dentro e uscirne in caso di guasto: toglierlo
  // sarebbe una scrittura in più che **può fallire a sua volta**, e allora
  // resterebbe in vendita e incompleto — esattamente lo stato che si vuole
  // impedire. Nascendo fuori, il guasto non ha bisogno che nulla vada a buon
  // fine per essere innocuo. *È la stessa logica per cui
  // `allergens_verified_at` si scrive DOPO e non prima.*
  //
  // -------------------------------------------------------------------------
  // ⚠️⚠️ LA COLONNA È `is_in_menu`, E FINO AL 13/08/2026 ERA `is_available`.
  // NON TORNARE INDIETRO CREDENDO DI SEMPLIFICARE.
  // -------------------------------------------------------------------------
  // **`is_available` era una difesa a scadenza, e la scadenza è l'alba.**
  // `app/api/cron/reset-availability/route.js` gira ogni giorno prima di
  // qualunque apertura ed esegue
  // `.update({ is_available: true }).eq("is_available", false)` su TUTTI i
  // prodotti: un articolo lasciato a metà da un guasto e "spento" così sarebbe
  // **tornato in vendita da solo l'indomani**, con metà opzioni e senza che
  // nessuno avesse fatto niente. *Il guasto sarebbe rimasto visibile per una
  // notte e poi sarebbe diventato silenzioso — cioè il contrario di ciò che
  // tutto questo blocco esiste per ottenere.*
  //
  // ⚠️ **`is_in_menu` il reset non la tocca mai**: nomina una colonna sola, ed è
  // la ragione per cui quella colonna è stata creata
  // (`sql/20260807_products_is_in_menu.sql`, 07/08/2026). Un articolo tolto dal
  // menu ci resta finché qualcuno non lo rimette.
  //
  // *Trovato leggendo il 13/08/2026, mentre si costruiva lo scudo della MODIFICA
  // delle opzioni (`lib/menu-options-editor.js`), che per la stessa ragione usa
  // `is_in_menu`. Il difetto c'era dal 12/08 e nessuna prova poteva vederlo: le
  ...
  // nasce acceso e in menu come dal 05/08/2026 (decisione 2), **né
  // `is_available` né `is_in_menu` vengono nominate**, nessuna delle quattro
  // tabelle viene sfiorata, e la Fase 3 si comporta esattamente come prima.
  // =========================================================================

  // 1) La riga. Né `is_available` né `is_in_menu` vengono impostate: il valore
  // predefinito del database è `true` per entrambe e la decisione 2 del
  // 05/08/2026 vuole che l'articolo nasca disponibile.
  // `allergens_verified_at` resta fuori di proposito (vedi sopra), e con esso i
  // flag dietetici, che si scrivono nello stesso passaggio 3.
  const riga = {
    store_id: storeId,
    category: clean.category,
    slug,
  ...
  // ⚠️ L'articolo CON OPZIONI nasce FUORI DAL MENU e ci rientra al passo 5.
  // Senza opzioni la riga non compare affatto e vale il valore predefinito del
  ...
  // lo stato finale di una creazione riuscita resta disponibile e in menu.
  // L'uscita dal menu dura quanto la sequenza, e sopravvive solo a un guasto.*
  //
  // ⚠️ `is_available` NON si tocca (13/08/2026): l'articolo nasce disponibile
  // come sempre, e ciò che lo tiene lontano dai clienti è l'altra colonna —
  // l'unica che il reset del mattino non rimette a `true`. Vedi il blocco qui
  // sopra.
  if (haOpzioni) {
    riga.is_in_menu = false;
  }
  ...
  const patch = {};
  // Vero solo dopo il rientro in menu del passo 5, e serve a comporre la
  // risposta senza toccare `patch`.
  let rientrato = false;
  if (!clean.bevanda) {
  ...
  //
  // ✅ `choice_label` ORA VIENE SCRITTO (decisione "YY" del 12/08/2026), uguale
  // su tutte le righe del gruppo, e col predefinito *"Come preferisci il tuo
  // kebab?"* invece di quello del database. ⚠️ *Fino a poche ore fa qui c'era
  // scritto che non veniva scritto affatto, e che un Roll creato dal pannello
  // avrebbe mostrato "Proteina": era il buco che questa riga chiude.*
  const righeOpzioni = [
    [
  ...
  for (const [tabella, righe] of righeOpzioni) {
    ...
    if (errOpzioni) {
      ...
      // ⚠️ Si esce SENZA rimettere l'articolo in menu e SENZA cancellarlo: resta
      // fuori dal menu, visibile nel pannello e irraggiungibile dai clienti. E
      // lo si dice a chi ha salvato, con la stessa forma dei due messaggi del
      // "mai verificato".
      //
      // ⚠️ Il messaggio dice "TOLTO DAL MENU" e non più "SPENTO" (13/08/2026):
      // dopo il passaggio a `is_in_menu` l'articolo non risulta esaurito, e
      // mandare Andrea a cercare il pulsante Disponibile gli farebbe premere il
      // pulsante sbagliato — che per giunta, su un articolo fuori menu, è spento.
      return errore(
        500,
        "Articolo creato, ma le sue opzioni non sono state salvate tutte: è rimasto TOLTO DAL MENU, quindi i clienti non lo vedono. Aprilo dal Menu, controlla le opzioni e rimettilo dentro con l'occhio."
      );
    }
  }

  // 5) IL RIENTRO IN MENU, che è l'ultimo atto prima del registro. Solo per gli
  // articoli nati fuori dal menu al passo 1, cioè quelli con opzioni.
  if (haOpzioni) {
    const { error: errRientro } = await db
      .from("products")
      .update({ is_in_menu: true })
      .eq("id", creato.id);
    if (errRientro) {
      console.error("[menu-create] Errore nel rimettere l'articolo in menu:", errRientro);
      // ⚠️ Il guasto meno grave della famiglia: l'articolo è completo, gli manca
      // solo di rientrare in menu. Resta fuori — visibile e innocuo — e si dice
      // dove premere.
      return errore(
        500,
        "Articolo creato con tutte le sue opzioni, ma è rimasto TOLTO DAL MENU: aprilo dal Menu e rimettilo dentro con l'occhio."
      );
    }
    ...
    // della verifica sembrava aver rimesso in menu l'articolo, e i due passi
  ...
  return {
    status: 201,
    body: {
      product: { ...creato, ...patch, ...(rientrato ? { is_in_menu: true } : {}) },
      allergens: etichette,
      sort_order_calcolato: clean.sort_order === null,
    },
  };
