...
// COMPATIBILITÀ TEMPORANEA (§30): il pannello invia ancora kind:"sauce" fino al
// secondo tempo. Entrambi i valori sono accettati e trattati come prodotto.
// DA RIMUOVERE quando il pannello invierà kind:"product" anche per le salse.
const ACCEPTED_KINDS = new Set(["product", "sauce"]);
...
export async function updateAllergensCore({ user, payload }) {
  ...
  if (!ACCEPTED_KINDS.has(payload.kind)) return fail(400, "Richiesta non valida.");
  const id = payload.id;
  ...
  const { error: logError } = await supabaseAdmin.from("staff_action_log").insert({
    ...
    detail: {
      // §30 (v32): un solo tipo di articolo, il prodotto. Registrato sempre
      // "product" a prescindere dal kind ricevuto (che in compatibilità può
      // ancora essere "sauce"), così il log non dipende dal passo temporaneo.
      kind: "product",
      item_id: id,
