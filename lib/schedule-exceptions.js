...
// `closingInclusive`: false = chiusura esclusa (Delivery §12, default storico);
// true = chiusura inclusa (Ritiro §12b, un ritiro all'orario di chiusura è
// valido). Su default il comportamento resta identico bit-per-bit.
function orderShift(scheduledDeliveryAt, windowsByDow, closingInclusive = false) {
  const { date, minuteOfDay, dayOfWeek } = romeParts(scheduledDeliveryAt);
  const wins = windowsByDow[dayOfWeek] ?? [];
  for (const w of wins) {
    const withinClose = closingInclusive
      ? minuteOfDay <= w.closesMinute
      : minuteOfDay < w.closesMinute;
    if (minuteOfDay >= w.opensMinute && withinClose) return { date, shift: w.shift };
  }
...
function filterAffectedOrders(orders, windowsByDow, { dateStart, dateEnd, closureType, excludedClosedSet = new Set() }) {
  ...
  for (const order of orders) {
    ...
    // §12b/§68.3: per il Ritiro la chiusura è inclusa (un ritiro all'orario di
    // chiusura è un ordine valido, quindi va conteggiato tra i colpiti); per la
    // Delivery resta esclusa (§12). Il flag è per-ordine, dalla sua modalità.
    const closingInclusive = order.fulfillment === "pickup";
    const { date, shift } = orderShift(order.scheduled_delivery_at, windowsByDow, closingInclusive);
    if (!shift) continue;
...
// §68.4/§46b — guard server-side (Task D): classifica uno scheduled_delivery_at
// rispetto a orari base (§13) ed eccezioni (§68). Ritorna il motivo del
// rifiuto così che la route possa scegliere la stringa §46b corrispondente,
// senza duplicare la logica di calcolo (vietato da §46b: due implementazioni
// divergono). Pura → testabile senza DB.
//  - "past"   → orario nel passato (o timestamp non valido)
//  - "closed" → fuori da ogni finestra §13, oppure turno chiuso da eccezione
//  - "ok"     → slot servibile
function classifyScheduledSlot(scheduledDeliveryAt, now, storeWindows, exceptions, closingInclusive = false) {
  const target = new Date(scheduledDeliveryAt).getTime();
  if (!Number.isFinite(target) || target <= new Date(now).getTime()) return "past";
  const windowsByDow = buildWindowsByDow(storeWindows);
  const { date, shift } = orderShift(new Date(scheduledDeliveryAt).toISOString(), windowsByDow, closingInclusive);
  if (!shift) return "closed";
  return closedShiftKeys(exceptions).has(`${date}|${shift}`) ? "closed" : "ok";
}

// §46b — la frase §46b che corrisponde a un verdetto di classifyScheduledSlot.
// Vive qui, accanto a chi quel verdetto lo produce, per la stessa ragione di
// `semaforoClosedMessage` e `checkoutBlockMessage`: il vocabolario chiuso
// ("past"/"closed"/"ok") e le frasi che lo traducono restano in un posto solo,
// e chi un domani aggiungesse un quarto verdetto trova subito la traduzione da
// scrivere. La route ci mette soltanto il 409 (§46b, convenzioni di errore).
//
// ⚠️ **Solo "past" è riconosciuto**: qualunque altro valore riceve il messaggio
// della chiusura, "ok" compreso. È il comportamento della route così com'è, e
// va preservato identico, non migliorato: nel percorso reale "ok" non arriva
// mai qui, perché chi chiama lo fa solo quando il verdetto è diverso da "ok".
function scheduledRejectionMessage(verdict) {
  return verdict === "past"
