...
function restoreCombo(ref, quantity, catalog) {
  ...
  // §23-26 (06/08/2026): se la bibita non è più disponibile il combo si toglie,
  // con lo stesso motivo degli articoli semplici e del Roll qui sopra. Il
  // catalogo la contiene ancora — riceve la lista PIENA, non quella filtrata che
  // alimenta il builder — proprio per poter distinguere "esaurita" da "sparita
  // dal menu" e scrivere il motivo giusto.
  if (matchedDrink.isAvailable === false) {
    return { removed: { id: roll.id, name: comboName, reason: REASON_UNAVAILABLE } };
  }
