...
function restoreCheckout(persisted) {
  ...
  // Lo sconto è un'INTENZIONE del cliente (§36-40 v41): si conserva la
  // richiesta, mai l'importo, che resta ricalcolato ogni volta. Nel dubbio non
  // si regala nulla: il ripiego è "non richiesto".
  if (isBoolean(persisted.giveMeFiveApplied)) {
    fields.giveMeFiveApplied = persisted.giveMeFiveApplied;
  } else {
    fields.giveMeFiveApplied = false;
    if (persisted.giveMeFiveApplied !== undefined) {
      dropped.push({ field: "giveMeFiveApplied", reason: DROP_INVALID });
    }
  }
