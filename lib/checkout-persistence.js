...
// §41-45 (11/08/2026): ⚠️ `country` è il paese scelto nella tendina dei
// prefissi, e si conserva come gli altri contatti — chi riapre il checkout
// ritrova il suo prefisso, non l'Italia rimessa d'ufficio sopra la sua scelta.
//
// ⚠️ **`FORMAT_VERSION` NON è stata alzata**, per la stessa ragione scritta più
// sotto per `codiceScritto`: alzarla butterebbe via il checkout conservato di
// chi sta ordinando in questo momento — indirizzo, contatti, orario — per una
// chiave in più. Una struttura salvata prima di oggi non porta `country`, e
// semplicemente non la ripristina: chi chiama fonde ciò che riceve con lo stato
// che ha già, dove l'Italia è il valore iniziale. Non è uno scarto, e c'è una
// prova che lo verifica su una struttura vecchia intera.
const CUSTOMER_DETAIL_KEYS = ["firstName", "lastName", "phone", "email", "country"];
