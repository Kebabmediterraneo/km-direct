...
export async function updateProductOptionsCore({
  ...
  for (const t of TABELLE) {
    ...
    if (error) {
      console.error(`[menu-options-editor] Errore lettura ${t.tabella}:`, error);
      return errore(500, "Errore interno. Riprova.");
    }
    attuali[t.tabella] = data ?? [];
  }
  ...
  if (proteineDiOggi.length > 0 && clean.proteins.length === 0) {
    return errore(
      400,
  ...
  if (scudoAlzato) {
    const { error: errRientro } = await db.from("products").update({ is_in_menu: true }).eq("id", id);
    if (errRientro) {
      console.error("[menu-options-editor] Errore nel rimettere l'articolo in menu:", errRientro);
      return errore(
        500,
        `Le opzioni di "${prodotto.name}" sono state salvate tutte, ma l'articolo è rimasto FUORI DAL MENU: aprilo dal Menu e rimettilo dentro con l'occhio.`
      );
    }
  }
