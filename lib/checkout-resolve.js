...
//
// ⚠️ "Togli dal menu" (spec v62): la lettura filtra DUE colonne, non una. Un
// articolo tolto dal menu va rifiutato dal server esattamente come uno esaurito
// — il browser è ciò che si vede, il server è ciò che vale. Non basta che la
// card sia sparita dalla griglia: una richiesta costruita a mano, o un carrello
// aperto da prima, arriverebbe qui lo stesso.
export async function resolveProduct(ref) {
  ...
  // blocco. proteinKey resta null se il prodotto non ha proteine.
  //
  // ⚠️⚠️ §17 (decisione di Andrea del 12/08/2026, "RR") — **SE IL PRODOTTO HA
  // DELLE PROTEINE, SCEGLIERNE UNA È OBBLIGATORIO.** Chi non vuole carne sceglie
  // *"nessuna"*, che è già fra i valori ammessi ed è **una scelta esplicita**.
  //
  // *La ragione: in cucina un ordine senza proteina non si distingue da un dato
  // perso. Chi prepara non sa se il cliente non l'abbia voluta o se qualcosa si
  // sia rotto per strada, e nel dubbio deve chiamare. "Nessuna" si distingue.*
  //
  // ⚠️ **Prima del 12/08 questo controllo NON C'ERA**: la proteina viveva dentro
  // un `if (ref.proteinLabel)` e, se non arrivava, il blocco veniva **saltato
  // senza che nessuno rifiutasse**. Non faceva danno solo perché il sito
  // preselezionava sempre qualcosa — una difesa che stava tutta nel browser,
  // cioè nel posto di cui §46 dice di non fidarsi.
  //
  // ⚠️ La forma è **la stessa dell'accompagnamento** poche righe più sotto, e lo
  // è di proposito: si leggono le righe del prodotto, e se ce ne sono si
  // pretende che la scelta corrisponda a una di esse. Due controlli gemelli si
  // leggono insieme; due controlli diversi che fanno la stessa cosa divergono.
  //
  // ⚠️ **Un prodotto SENZA proteine non cambia comportamento**: l'Egiziano e il
  // Cipriota (§19) non hanno righe qui e restano ordinabili esattamente come
  // prima. E se una proteina arrivasse per un prodotto che non ne ha, resta
  // rifiutato com'era già oggi — la vecchia lettura filtrata non trovava nulla e
  // cadeva sul `return null`.
  //
  // ⚠️ *Limite ereditato, non introdotto qui: un guasto di lettura fa arrivare
  // `undefined` e questo prodotto sembra "senza proteine". È esattamente ciò che
  // già accade all'accompagnamento, e cambiarlo per la sola proteina
  // significherebbe rendere i due gemelli diversi.*
  ...
  if (choices && choices.length > 0) {
    const choice = choices.find((c) => c.label === ref.proteinLabel);
...
export async function resolveCombo(ref, storeId) {
  ...
  // ⚠️ "Togli dal menu" (spec v62): come in `resolveProduct`, due colonne. Un
  // Roll tolto dal menu non deve restare acquistabile nemmeno per la strada del
  // combo — stesso prodotto, percorso diverso.
  ...
  // ⚠️ §17 (12/08/2026, "RR") — **STESSO BUCO, STESSA CHIUSURA.** Il combo aveva
  // la proteina dentro un `if (ref.proteinLabel)` esattamente come il prodotto
  // singolo: se non arrivava, nessuno rifiutava. Le proteine del combo sono
  // quelle del **Roll scelto**, quindi si guardano le righe di `rollProductId`.
  //
  // *Chiuderlo in un solo posto sarebbe stato peggio che non chiuderlo: il buco
  // sarebbe rimasto sulla metà del file che nessuno guarda, e la prova sull'altra
  // metà avrebbe dato l'impressione che la cosa fosse a posto.*
  let proteinSurcharge = null;
  const { data: choices } = await supabaseAdmin
    .from("product_choice_options")
    .select("*")
  ...
  if (ref.drinkProductId) {
    ...
    // §23-26 (06/08/2026): la disponibilità che conta per la bibita è quella del
    // PRODOTTO. La lettura qui sopra filtra `combo_drink_options.is_available`,
    // cioè la colonna di QUELLA tabella, che nessuna schermata del pannello
    // scrive: da sola lasciava passare una bibita segnata esaurita, che veniva
    // venduta dentro il combo mentre la sua card mostrava il pulsante spento.
    // Stessa forma delle due letture che questo file già fa — il prodotto
    // singolo in `resolveProduct` e il Roll poche righe sopra: si legge
    // `products` per id con `is_available` vero, e l'assenza rifiuta la riga.
    //
    // ⚠️ "Togli dal menu" (spec v62): e con `is_in_menu` vero, terza e ultima
    // delle tre letture su `products` di questo file. Una bibita ritirata dal
    // menu non deve poter essere venduta dentro un combo, come non lo è una
    // esaurita.
    const { data: drinkProduct } = await supabaseAdmin
      .from("products")
      .select("*")
      .eq("id", ref.drinkProductId)
      .eq("is_available", true)
      .eq("is_in_menu", true)
      .single();
